...
IGL_INLINE bool collapse_edge(const int e, const Eigen::RowVectorXd &p,
    ...
    for (auto f : nV2Fd) {
        for (int v = 0; v < 3; v++) {
            if (F(f, v) == d) {
                const int flip1 =
                    (EF(EMAP(f + m * ((v + 1) % 3)), 0) == f) ? 1 : 0;
                const int flip2 =
                    (EF(EMAP(f + m * ((v + 2) % 3)), 0) == f) ? 0 : 1;
...
IGL_INLINE bool collapse_edge(
    ...
    if (collapsed) {
        ...
        // reinsert with infinite weight (the provided cost function must
        // **not**
